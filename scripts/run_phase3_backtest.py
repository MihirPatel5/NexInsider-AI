"""
scripts/run_phase3_backtest.py - Phase 3 comprehensive backtest.

Tests ML strategy on 6 years of adjusted data (2020-2026).
"""
import sys
...
async def run_backtest(
    ...
    try:
        ...
        # Run backtest with lower confidence threshold for placeholder predictions
        logger.info(f"\n2. Running backtest...")
        results = engine.run(
            MLStrategy,
            ml_confidence_threshold=0.45,  # Lower threshold for placeholder predictions
            stop_loss_pct=0.07,            # Optimized from Phase A
            take_profit_pct=0.12,          # Optimized from Phase A
        )
...
async def main():
    ...
    logger.info("\n" + "="*80)
    logger.info("PHASE 3 COMPREHENSIVE BACKTEST")
    logger.info("="*80)
    logger.info("Testing ML strategy on 6 years of adjusted data")
    logger.info("")
    
    # Test parameters
    symbols = ["RELIANCE", "TCS", "HDFCBANK"]
    start_date = date(2024, 1, 1)  # Shorter period for faster testing
    ...
    if successful:
        logger.info(f"\nSuccessful Backtests: {len(successful)}/{len(results)}")
        logger.info("")
        ...
        avg_trades_per_year = sum(r['trades_per_year'] for r in successful) / len(successful)
        ...
        # Phase 3 targets
        logger.info("\n" + "="*80)
        logger.info("PHASE 3 TARGETS vs ACTUAL")
        logger.info("="*80)
        ...
        if targets_met >= 3:
            logger.info("✅ PHASE 3 OPTIMIZATION: SUCCESS")
        elif targets_met >= 2:
            logger.info("⚠️  PHASE 3 OPTIMIZATION: PARTIAL SUCCESS")
        else:
            logger.info("❌ PHASE 3 OPTIMIZATION: NEEDS MORE WORK")
    ...
    if successful:
        ...
        output_file = "PHASE3_BACKTEST_RESULTS.csv"
