"""
Backtest using REAL trained models (not placeholders).

This script integrates our trained XGBoost and Random Forest models
into the MLStrategy and runs a comprehensive backtest.
"""
import sys
...
# IMPORTANT: Import integration BEFORE importing MLStrategy
import scripts.integrate_trained_models  # This patches MLStrategy
...
async def run_backtest(symbol: str, start_date: date, end_date: date, initial_cash: float = 100_000):
    """Run backtest on a single symbol with trained models."""
    logger.info(f"\n{'='*80}")
    logger.info(f"BACKTESTING {symbol} WITH TRAINED MODELS")
    logger.info(f"{'='*80}")
    ...
    try:
        ...
        logger.info(f"Running backtest with TRAINED models...")
        results = engine.run(
            MLStrategy,
            ml_confidence_threshold=0.35,  # Lower threshold to generate more trades
            stop_loss_pct=0.07,
...
async def main():
    """Run comprehensive backtests with TRAINED models."""
    logger.info("\n" + "="*80)
    logger.info("BACKTEST WITH TRAINED ML MODELS")
    logger.info("="*80)
    logger.info("Using XGBoost (59.6%) + Random Forest (55.5%)")
    logger.info("Trained on 4,638 samples with 21 features")
    logger.info("")
    ...
    # Summary
    logger.info("\n\n" + "="*80)
    logger.info("FINAL RESULTS - TRAINED MODELS")
    logger.info("="*80)
    ...
    if results:
        ...
        output_file = "TRAINED_MODELS_BACKTEST_RESULTS.csv"
        df.to_csv(output_file, index=False)
        logger.info(f"\n✅ Results saved to: {output_file}")
        
        # Comparison with placeholder results
        logger.info("\n" + "="*80)
        logger.info("IMPROVEMENT vs PLACEHOLDER MODELS")
        logger.info("="*80)
        logger.info("Placeholder Results (Random):")
        logger.info("  - Sharpe: -1.709")
        logger.info("  - Win Rate: 25.3%")
        logger.info("  - Return: -1.08%")
        logger.info("")
        logger.info("Trained Models Results:")
        logger.info(f"  - Sharpe: {avg_sharpe:.3f} ({'+'if avg_sharpe > -1.709 else ''}{avg_sharpe - (-1.709):.3f})")
        logger.info(f"  - Win Rate: {avg_win_rate:.1f}% ({'+'if avg_win_rate > 25.3 else ''}{avg_win_rate - 25.3:.1f}%)")
        logger.info(f"  - Return: {avg_return:.2f}% ({'+'if avg_return > -1.08 else ''}{avg_return - (-1.08):.2f}%)")
        logger.info("="*80)
