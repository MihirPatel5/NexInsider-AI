"""
Fast backtest using trained models - optimized version.
"""
import sys
...
def patch_ml_strategy_optimized():
    """Optimized ML strategy using pre-calculated features."""
    ...
    MLStrategy._feature_engineer = FeatureEngineer()
    MLStrategy._features_df = None
    
    # Override __init__ to pre-calculate features
    original_init = MLStrategy.__init__
    
    def new_init(self):
        original_init(self)
        # Pre-calculate all features once
        if MLStrategy._features_df is None:
            logger.info("Pre-calculating features...")
            df = self._get_full_dataframe()
            if df is not None and len(df) > 50:
                MLStrategy._features_df = self._feature_engineer.extract_features(df)
                logger.info(f"✅ Pre-calculated {len(MLStrategy._features_df)} feature rows")
    
    def _get_full_dataframe(self):
        """Get full dataframe for pre-calculation."""
        try:
            data = self.datas[0]
            bars = len(data)
    ...
    def new_get_ml_prediction(self):
        """Get ML prediction using pre-calculated features."""
        try:
            if MLStrategy._features_df is None or MLStrategy._features_df.empty:
                logger.warning("Features DF is None or empty")
                return None
            
            # Get current bar index
            current_idx = len(self.datas[0]) - 1
            
            if current_idx >= len(MLStrategy._features_df):
                logger.warning(f"Index {current_idx} >= features length {len(MLStrategy._features_df)}")
                return None
            
            # Get features for current bar
            latest_features = MLStrategy._features_df.iloc[current_idx]
            ...
            confidence = (xgb_conf + rf_conf) / 2.0
            
            # Debug first few predictions
            if current_idx < 5:
                logger.info(f"Bar {current_idx}: xgb={xgb_pred}, rf={rf_pred}, avg={avg_pred:.2f}, signal={signal}, conf={confidence:.2f}")
            
            return {
                "signal": signal,
                "confidence": confidence,
                "buy_prob": (xgb_proba[2] + rf_proba[2]) / 2.0,
                "sell_prob": (xgb_proba[0] + rf_proba[0]) / 2.0,
                "hold_prob": (xgb_proba[1] + rf_proba[1]) / 2.0,
            }
    ...
    # Patch methods
    MLStrategy.__init__ = new_init
    MLStrategy._get_ml_prediction = new_get_ml_prediction
    MLStrategy._get_full_dataframe = _get_full_dataframe
    
    logger.info("✅ MLStrategy patched (optimized)")
...
async def run_backtest(symbol: str, start_date: date, end_date: date, initial_cash: float = 100_000):
    ...
    try:
        # Reset feature cache for new symbol
        MLStrategy._features_df = None
...
async def main():
    ...
    logger.info("\n" + "="*80)
    logger.info("BACKTEST WITH TRAINED ML MODELS (OPTIMIZED)")
    logger.info("="*80)
    
    # Patch strategy
    patch_ml_strategy_optimized()
    ...
    if results:
        ...
        output_file = "TRAINED_ML_BACKTEST_RESULTS.csv"
